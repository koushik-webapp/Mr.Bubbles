...
export const Story = ({ className, ...props }: StoryProps) => (
  <CarouselItem className={cn('basis-auto !w-[200px] pl-2 md:pl-4', className)}>
    <div
      className={cn(
        'group relative overflow-hidden rounded-xl bg-[#0D1B4A]/40',
...
export const StoryVideo = ({ className, ...props }: StoryVideoProps) => {
  ...
  const handleMouseOut = () => {
    if (videoRef.current) { videoRef.current.pause(); videoRef.current.currentTime = initialTimeRef.current; }
  };
  ...
  return (
    <video
      ...
      loop muted
      onBlur={handleBlur} onFocus={handleFocus} onMouseOut={handleMouseOut} onMouseOver={handleMouseOver}
      preload="metadata"
      ref={videoRef}
      tabIndex={0}
      {...props}
...
export const StoryOverlay = ({ className, side = 'bottom', ...props }: StoryOverlayProps) => {
  ...
  return (
    <div
      className={cn('absolute right-0 left-0 h-16 from-black/50 to-transparent', positionClasses, className)}
      {...props}
    />
  );
};
